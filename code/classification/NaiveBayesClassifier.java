...
public class NaiveBayesClassifier implements Classifier {
	...
	private static Logger logger = Logger
			.getLogger(NaiveBayesClassifier.class.getPackage()
					.getName());
	static {
	...
	private Map<String, Integer> generalLabelCounts = new HashMap<String, Integer>();
	private ArrayList<Map<String, Map<String, Integer>>> attrValLabelCounts = new ArrayList<Map<String, Map<String, Integer>>>();

	/**
	 * Builds a classifier using the given training data
	 * 
	 * @param trainingData
	 *                a list of {@code DataVector}s
	 */
	public NaiveBayesClassifier(List<DataVector> trainingData) {
		...
		for (int v = 0; v < trainingData.size(); v++) {
			...
			for (int attrIdx = 0; attrIdx < numAttributes; attrIdx++) {
				...
				if (isFirstVector) {
					...
					attrValLabelCounts
							.add(valueLabelCounts);
				}
				valueLabelCounts = attrValLabelCounts
						.get(attrIdx);

				Map<String, Integer> labelCounts = valueLabelCounts
						.get(attrVal);
				if (labelCounts == null) {
					labelCounts = new HashMap<String, Integer>();
					valueLabelCounts.put(attrVal,
							labelCounts);
				}
		...
		for (Entry<String, Integer> entry : generalLabelCounts
				.entrySet()) {
		...
		for (int i = 0; i < attrValLabelCounts.size(); i++) {
			for (Entry<String, Map<String, Integer>> entry : attrValLabelCounts
				...
				for (Entry<String, Integer> count : entry
						.getValue().entrySet()) {
					logger.finer(String
							.format("AttrIdx:%d,attrVal:%s,class:%s: %d",
									i,
									attrVal,
									count.getKey(),
									count.getValue()));
	...
	public String classify(DataVector vector) {
		...
		for (Entry<String, Integer> entry : generalLabelCounts
			...
			for (int k = 0; k < data.length; k++) {
				...
				Map<String, Map<String, Integer>> attrKCounts = attrValLabelCounts
						.get(k);
				Map<String, Integer> valCounts = attrKCounts
						.get(data[k]);

				int numMatchingValLabel = 0;
				int totalMatchingLabel = numMatchingLabel;
				if (valCounts != null
						&& valCounts.containsKey(label)) {
					numMatchingValLabel = valCounts
							.get(label);
				} else {
